...
public class MaliciousActivityDetector implements Runnable {
    ...
    public MaliciousActivityDetector(ServerConfigurator configurator) {
        this.configurator = configurator;
    }

    @Override
    public void run() {
        MongoDatabase db = DatabaseConnectionManager.getDatabase();
                ...
                try {
                    ...
                    for (Map.Entry<String, PeerDB> peerDBEntry: peerDBMap.entrySet()) {
                        if (peerDBEntry.getValue().isActive()) {
                            ...
                            PeerInfo peerInfo = new PeerInfo("FDS", configurator.getIntProperty("FDS_PORT"));
                            ...
                            Socket peerSocket = new Socket(configurator.getProperty("IP_ADDRESS"), peerDB.getPort_no());
                                ...
                                for (String file: unTraceableFiles) {
                                    ...
                                    for (String replicatedPeerId: replicatedPeerIds) {
                                        if (!replicatedPeerId.equals(peerId)) {
                                            ...
                                            Socket ownerPeerSocket = new Socket(configurator.getProperty("IP_ADDRESS"), replicatedPeerInfo.getPort_no());
